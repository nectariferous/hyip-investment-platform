            ...
            <div class="auth-body-part">
                <div class="auth-form-wrapper">
                    ...
                    <form action="{{ route('user.forgot.password') }}" method="POST">
                        ...
                        @endif
                        <div class="mb-3">
                            <button type="submit" id="recaptcha" class="sp_theme_btn w-100">{{ __('Send Verification Code') }}</button>
                        </div>
                        <div>
                            <p class="text-center">{{ __('Login Again') }}? <a href="{{ route('user.login') }}" class="color-change" >{{ __('Login') }}</a></p>
                        </div>
                    </form>
                </div>
            </div>
